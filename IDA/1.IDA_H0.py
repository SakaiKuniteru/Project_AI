...
# Function to solve the TSP problem using IDA* algorithm with a heuristic function h(n) = 0
    ...
    def dfs(node, path, cost, visited):
        ...
        for next_node in range(num_cities):
            if next_node not in visited and cost_matrix[node][next_node] > 0:
                # Calculate the cost and heuristic for the next node
                new_cost = cost + cost_matrix[node][next_node]
                heuristic = 0
                total_estimate = new_cost + heuristic

                # Check if the estimated cost is lower than the current optimal cost
